	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < inst.size(); ++i) {
			...
			if (j.startsWith("jnz")) {
				...
				if (c1 >= 97 && c1 <= 100) {
					v = r[j.split(" ")[1].charAt(0) - 97];
				} else {
					v = Integer.parseInt(j.split(" ")[1]);
				}
				if (v != 0) {
					i += Integer.parseInt(j.split(" ")[2]) - 1;
				}
				...
				if (c1 >= 97 && c1 <= 100) {
					...
					r[c2 - 97] = Integer.parseInt(j.split(" ")[1]);
				}
			} else if (j.startsWith("inc")) {
				r[j.split(" ")[1].charAt(0) - 97]++;
