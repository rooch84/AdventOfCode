...
/**
 * Starting state is 10010000000110000
 * 
 ...
 * single 0, then "b". For example, after a single step of this process,
 * 
 * 1 becomes 100. 0 becomes 001. 11111 becomes 11111000000. 111100001010 becomes
 * 1111000010100101011110000. Repeat these steps until you have enough data to
 * fill the desired disk.
 * 
 ...
 * characters long (odd), so we are done. In this example, the correct checksum
 * would therefore be 01100.
 * 
...
public class Day16 {
	...
	public static void main(String[] args) {
		...
		if (args.length != 2) {
			System.out.println("Incorrect number of arguments. Correct usage is <disc size> <initial state>");
			System.exit(0);
		}

		int length = Integer.parseInt(args[0]);
		BitSet disc = new BitSet(20);

		String input = args[1];
		int l = input.length();
		for (int i = 0; i < input.length(); ++i) {
			disc.set(i, input.charAt(i) == '1' ? true : false);
		}
		expand(disc, l, length);
		Checksum c = check(disc, length);
		printBits(c.b, c.l);
	}

	private static Checksum check(BitSet input, int l) {
		...
		if (cLength % 2 == 0) {
			return check(s, cLength);
		}

		return new Checksum(s, cLength);
	}

	private static void expand(BitSet disc, int l, int tL) {

		BitSet rev = new BitSet(l);

		for (int i = l - 1; i >= 0; --i) {
			rev.set(l - 1 - i, disc.get(i));
		}
	...
	public static void printBits(BitSet b, int l) {
		for (int i = 0; i < l; ++i) {
	...
	private static class Checksum {
		...
		public Checksum(BitSet b, int l) {
			this.l = l;
			this.b = b;
		}
	}
