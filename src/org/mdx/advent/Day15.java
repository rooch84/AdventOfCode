...
public class Day15 {
	...
	public static void main(String[] args) {
		...
		List<Disc> d = new ArrayList<Disc>();
		while (scn.hasNextLine()) {
			String line = scn.nextLine().replace(".", "");
			int p = Integer.parseInt(line.split(" ")[3]);
			int s = Integer.parseInt(line.split(" ")[11]);
			d.add(new Disc(p, s));
		}
		...
		while (notAligned) {
			...
			for (int i = 0; i < d.size(); ++i) {
				long aligned = d.get(i).s + (t + i + 1) % d.get(i).p;
				if (aligned >= d.get(i).p) {
					aligned -= d.get(i).p;
				}
				if (aligned != 0) {
					notAligned = true;
